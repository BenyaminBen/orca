...
test.skip(process.platform !== 'win32', 'Requires a real Windows WSL host')
  ...
  scenarioTest(
    ...
    async ({ orcaPage: page, electronApp, registerPostElectronShutdownCleanup }, testInfo) => {
      ...
      try {
        ...
        const folderRow = row('Nested folder')
        ...
        const expectedPath = path.win32.join(folders.workspacePath, SELECTED_FOLDER)
        await (scenario.location === 'drive'
          ? expect
              .poll(async () => {
                const actual = await folderRow.getAttribute('data-native-file-drop-dir')
                return actual === null ? null : path.win32.normalize(actual)
              })
              .toBe(expectedPath)
          : expect(folderRow).toHaveAttribute('data-native-file-drop-dir', expectedPath))
        await folderRow.click()
        await expect(row(CHILD_FILE)).toBeVisible()
        if (scenario.reject) {
          ...
          await expect(folderRow).toHaveAttribute('data-selected', 'true')
          await expect(row(CHILD_FILE)).toBeVisible()
          await expect(row('Outside folder')).toHaveCount(0)
        }
