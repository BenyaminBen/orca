...
export async function createNativeChatWslFolders(
  ...
  const wslVersion = row?.at(-1)
  expect(['1', '2'], `Expected WSL1 or WSL2 for ${distro}: ${inventory}`).toContain(wslVersion)
  const requiredVersion = process.env.ORCA_E2E_WSL_VERSION
  if (requiredVersion !== undefined) {
    expect(wslVersion, `Expected WSL${requiredVersion} for ${distro}: ${inventory}`).toBe(
      requiredVersion
    )
  }
  record({ testedSha, distro, wslVersion: Number(wslVersion) })
  ...
  const linuxRoot =
    location === 'drive'
      ...
      : await guest(['/usr/bin/mktemp', '-d', '/tmp/orca-native-chat-XXXXXX'])
