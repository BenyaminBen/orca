...
beforeEach(() => {
  ...
  delete mocks.state.settings.terminalLinkClickBehavior
...
describe('file destinations in chat responses', () => {
  ...
  it.each(['actions', 'open', 'none'] as const)('honors the %s click preference', (behavior) => {
    render(<Transcript />)
    mocks.state.settings.terminalLinkClickBehavior = behavior
    fireEvent.click(screen.getByRole('link'))
    expect(Boolean(screen.queryByRole('button', { name: /Open in Orca/ }))).toBe(
      behavior === 'actions'
    )
    expect(mocks.openDetectedFilePath).toHaveBeenCalledTimes(behavior === 'open' ? 1 : 0)
    expect(mocks.reveal).not.toHaveBeenCalled()
