...
const mocks = vi.hoisted(() => {
  const settings: Pick<
    GlobalSettings,
    'terminalLinkActionPopoverEnabled' | 'terminalLinkClickBehavior'
...
beforeEach(() => {
  ...
  delete mocks.settings.terminalLinkClickBehavior
...
describe('transcript image destinations', () => {
  it.each([true, false])(
    'preserves duplicate entries with preview mode %s',
    async (enablePreview) => {
      ...
      if (enablePreview) {
        expect(await screen.findAllByRole('img', { name: 'generated.png' })).toHaveLength(2)
      } else {
        expect(screen.getAllByText('generated.png')).toHaveLength(2)
  ...
  it.each(['actions', 'open', 'none'] as const)(
    'honors the %s image click preference',
    async (behavior) => {
      render(<ImageMessage />)
      await screen.findByRole('img')
      mocks.settings.terminalLinkClickBehavior = behavior
      fireEvent.click(screen.getByRole('button', { name: 'View image: generated.png' }))
      expect(Boolean(screen.queryByRole('button', { name: /Open in Orca preview/ }))).toBe(
        behavior === 'actions'
      )
      expect(Boolean(screen.queryByRole('dialog', { name: 'generated.png' }))).toBe(
        behavior === 'open'
      )
      expect(mocks.openFilePath).not.toHaveBeenCalled()
      expect(mocks.downloadAndOpenRemoteTerminalFile).not.toHaveBeenCalled()
    }
  )
