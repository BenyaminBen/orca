...
export async function openCodexThreadRestoringPermissions(input: {
  ...
  let defaultPolicy =
    !savedPolicy && launch.permissionPolicy?.sandbox === 'danger-full-access'
      ? codexPermissionPolicy('full-access')
      : undefined
  const requestedPolicy = savedPolicy ?? defaultPolicy
  const desired =
  ...
  if (isCodexPermissionMode(desired) && (recovery || desired !== 'ask-for-approval')) {
    ...
    if (requested?.restrictedByHost) {
      ...
      if (defaultPolicy) {
        defaultPolicy = requiredFallback
      } else {
  ...
  const opened = await openCodexThread(
    ...
    {
      ...launch,
      permissions: recovery?.effective ?? savedPolicy ?? defaultPolicy
    },
    timeoutMs
  )
  assertCurrent()
